...
describe('UserService', () => {
  ...
  beforeEach(async () => {
    ...
    //console.log('userEntityMock: ', userEntityMock);
  });
  ...
  it('should return Erro in findUserByEmail', async () => {
    jest.spyOn(userRepository, 'findOne').mockResolvedValue(undefined);

    expect(service.findUserByEmail(userEntityMock.email)).rejects.toThrowError();
  });
  
  it('should return Erro in findUserByEmail (error DB)', async () => {
  ...
  it('should return Erro in findUserById', async () => {
    jest.spyOn(userRepository, 'findOne').mockResolvedValue(undefined);

    expect(service.findUserById(userEntityMock.id)).rejects.toThrowError();
  });

  it('should return Erro in findUserById (error DB)', async () => {
    jest.spyOn(userRepository, 'findOne').mockRejectedValueOnce(new Error());
